...
import scala.collection.{ArrayOps, StringOps, immutable, mutable}
...
private[tms] trait TmsImplicits extends TmsHigherPriorityImplicits {
  ...
  val fromScalaVersionIncluding: Option[String] = Some("2.13.0")
  val toScalaVersionExcluding: Option[String] = Some("3.0.0")
  ...
  trait TmsStringImplicitExtensions { // valid for Java 16 inclusive
    ...
    // StringOps
    def concat[B >: Char](suffix: scala.collection.IterableOnce[B]): IndexedSeq[B] // String native + StringOps & WrappedString implicit overloads
    def concat(suffix: scala.collection.IterableOnce[Char]): String
    def contains(elem: Char): Boolean // String native + StringOps & WrappedString implicit overloads
    def split(separator: Char): Array[String] // String native + StringOps implicit overloads
    def split(separators: Array[Char]): Array[String]
    // WrappedString
    def startsWith[B >: Char](that: scala.collection.IterableOnce[B], offset: Int): Boolean // String native + WrappedString implicit overloads
    def startsWith[B >: Char](that: scala.collection.IterableOnce[B]): Boolean
    def endsWith[B >: Char](that: Iterable[B]): Boolean // String native + WrappedString implicit overloads
    def indexOf[B >: Char](elem: B, from: Int): Int // String native + WrappedString implicit overloads
    def indexOf[B >: Char](elem: B): Int
    def lastIndexOf[B >: Char](elem: B, end: Int): Int // String native + WrappedString implicit overloads
    def lastIndexOf[B >: Char](elem: B): Int
    def contains[A1 >: Char](elem: A1): Boolean
    //  defs `stepper`, `copyToArray`, `updated` implicit extensions each have only StringOps & WrappedString implicit overloads (no String native ones)
    //  but due to StringOps first priority typed WrappedString overloads do not work at all (even without tts) that is STRANGE (`contains` has String native + both & both works!):
    // `"123".copyToArray(Array[Any](3))` errors with:
    //type mismatch;
    //found   : Array[Any]
    //required: Array[Char]
    ...
    // But! with explicit wrapString: `wrapString("123").copyToArray(Array[Any](3))` it compiles & works correctly
    //TO-DO try in Scala 3.0
  }
...
private[tms] trait TmsHigherPriorityImplicits extends TmsMiddlePriorityImplicits {
  ...
  // above TmsImplicits.refArrayOps has. This is also required to resolve ambiguous implicits on Arrays with dimensions >= 2 and do not break others.
...
private[tms] trait TmsLowestPriorityImplicits {
  ...
  @replaceByTtsImplicits @compileTimeOnly(ctom) def genericWrapArray[T](t: Array[T]): ArraySeq[T]
  ...
  // Tn <: AnyRef is required to make priority order of tts1xxx-tts5xxx groups. tts5xxx has the highest priority
  ...
  // Predef.LowPriorityImplicits2 is not included since the only implicit method copyArrayToImmutableIndexedSeq it contains is deprecated
